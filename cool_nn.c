...
cool_nn* cool_alloc(const char* nn_config) {
    ...
    for (int i = 0; i < net->layers_n; i++) {
        ...
        bool out_layer = i == (net->layers_n - 1) ? true : false;
        int in_c, out_c, stride, f_size, padd, in_w, in_h, fc_size, activ;

        net->layers_type[i] = type;
        if (type == CONV) {
            fscanf(cfg_fp, " %d %d %d %d %d %d %d %s\n", &in_c, &out_c, &stride, &f_size, &padd, &in_w, &in_h, activ_name);
            activ = dec_activation(activ_name);
            net->layers[i] = conv_alloc(in_c, in_w, in_h, out_c, f_size, stride, padd, activ, out_layer);
        }
        else if (type == FC) {
            fscanf(cfg_fp, "%d %d %s\n", &in_c, &fc_size, activ_name);
            activ = dec_activation(activ_name);
            net->layers[i] = fc_alloc(in_c, fc_size, activ, out_layer);
        }
...
cool_nn* cool_load(const char* nn_config, const char* nn_state) {
    ...
    for (int i = 0; i < net->layers_n; i++) {
        if (net->layers_type[i] == CONV) {
            ...
            fread(l->gamma->data, sizeof(float), l->out_c, nn_state_fp);
            fread(l->beta->data, sizeof(float), l->out_c, nn_state_fp);
            fread(l->run_mean->data, sizeof(float), l->out_c, nn_state_fp);
            fread(l->run_var->data, sizeof(float), l->out_c, nn_state_fp);
        }
            ...
            fread(l->gamma->data, sizeof(float), out_dim, nn_state_fp);
            fread(l->beta->data, sizeof(float), out_dim, nn_state_fp);
            fread(l->run_mean->data, sizeof(float), out_dim, nn_state_fp);
            fread(l->run_var->data, sizeof(float), out_dim, nn_state_fp);
        }
    }
...
void cool_save(cool_nn *net, const char* nn_state) {
    ...
    for (int i = 0; i < net->layers_n; i++) {
        if (net->layers_type[i] == CONV) {
            ...
            fwrite(l->gamma->data, sizeof(float), l->out_c, nn_state_fp);
            fwrite(l->beta->data, sizeof(float), l->out_c, nn_state_fp);
            fwrite(l->run_mean->data, sizeof(float), l->out_c, nn_state_fp);
            fwrite(l->run_var->data, sizeof(float), l->out_c, nn_state_fp);
        }
        else if (net->layers_type[i] == FC) {
            fc_layer *l = (fc_layer*)net->layers[i];
            int in_dim = l->weights->rows, out_dim = l->weights->columns;
            ...
            fwrite(l->gamma->data, sizeof(float), out_dim, nn_state_fp);
            fwrite(l->beta->data, sizeof(float), out_dim, nn_state_fp);
            fwrite(l->run_mean->data, sizeof(float), out_dim, nn_state_fp);
            fwrite(l->run_var->data, sizeof(float), out_dim, nn_state_fp);
        }
    }
...
matrix* cool_forward(cool_nn *net, matrix *batch, bool training) {
    ...
    matrix *curr = mat_copy(batch), *in;
    
    for (int i = 0; i < net->layers_n; i++) {
        in = mat_copy(curr);
        matrix_free(curr);
        if (net->layers_type[i] == CONV) {
            curr = conv_forward((conv_layer*)net->layers[i], in, training);
        }
        else if (net->layers_type[i] == FC) {
            curr = fc_forward((fc_layer*)net->layers[i], in, training);
        }
        else if (net->layers_type[i] == MAX_POOL) {
...
void cool_backward(cool_nn *net, matrix *prob, matrix *batch, int *indices, int *labels, float l_rate, float l_reg) {
    ...
    matrix *in;
    matrix *curr = prob_del(prob, indices, labels);
